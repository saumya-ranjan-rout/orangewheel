...
<?php echo form_open(site_url('hr/payroll_selector')); ?>
                        ...
                        <optgroup label="<?php echo get_phrase($account_type); ?>">
                            ...
                            foreach ($user_info as $row) { ?>
                                ...
                                <option value="<?php echo $account_type . '-' . $row[$account_type . '_id']; ?>"
                                    ...
                                    - <?php echo $row['name']; ?>(<?php echo $row['staff_id']; ?>)</option>
...
<?php echo form_open(site_url('hr/create_payroll'),
        ...
        <div class="col-md-offset-1 col-md-10">
            <div class="panel panel-primary" data-collapsed="0">
                ...
                <div class="panel-body ">
                    <?php
     if ($user_type === 'doctor') {
    $doctor_info = $this->db->get_where('doctor', array('doctor_id' => $employee_id))->row();
    $basic_salary = $doctor_info->basic_salary;
} elseif ($user_type === 'accountant') {
    $accountant_info = $this->db->get_where('accountant', array('accountant_id' => $employee_id))->row();
    $basic_salary = $accountant_info->basic_salary;
}elseif ($user_type === 'nurse') {
    $accountant_info = $this->db->get_where('nurse', array('nurse_id' => $employee_id))->row();
    $basic_salary = $accountant_info->basic_salary;
}elseif ($user_type === 'pharmacist') {
    $accountant_info = $this->db->get_where('pharmacist', array('pharmacist_id' => $employee_id))->row();
    $basic_salary = $accountant_info->basic_salary;
}elseif ($user_type === 'laboratorist') {
    $accountant_info = $this->db->get_where('laboratorist', array('laboratorist_id' => $employee_id))->row();
    $basic_salary = $accountant_info->basic_salary;
}elseif ($user_type === 'receptionist') {
    $accountant_info = $this->db->get_where('receptionist', array('receptionist_id' => $employee_id))->row();
    $basic_salary = $accountant_info->basic_salary;
} else {
    $basic_salary = 0;
}?>
                    <div class="form-group">
                        <label for="field-1" class="col-sm-3 control-label"><?php echo get_phrase('basic'); ?> <small style="color:red;">*</small></label>

                        <div class="col-sm-7">
                            <input type="text" class="form-control" name="joining_salary" id="basic" value="<?php echo $basic_salary;?>" required readonly />
                        </div>
                    </div>
...
<script type="text/javascript">
    ...
    function get_employees(department_id)
    {
        if(department_id != '')
            $.ajax({
                url     : '<?php echo site_url('hr/get_employees/'); ?>' + department_id,
