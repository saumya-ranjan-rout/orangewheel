...
function confirm_modal(delete_url) {
    ...
    var modal = jQuery('#modal-4');

    // Adjust the modal position based on the scroll position
    modal.css('top', scrollPosition + 100 + 'px');
    ...
    document.getElementById('delete_link').addEventListener('click', function(event) {
        event.preventDefault(); // Prevent the default link behavior

        // Show an additional confirmation before proceeding with the deletion
        var confirmDelete = confirm('Are you sure you want to delete this information? This action cannot be undone.');

        if (confirmDelete) {
            // If user confirms, proceed with the deletion by navigating to the delete URL
...
<script type="text/javascript">
    function staus_confirm_modal(update_url) {
...
<div class="modal fade" id="modal-5">
    <div class="modal-dialog">
        <div class="modal-content" style="margin-top:100px;">
            ...
            <div class="modal-footer" style="margin:0px; border-top:0px; text-align:center;">
                <a href="#" class="btn btn-primary" id="update_link"><?php echo get_phrase('update'); ?></a>
                <button type="button" class="btn btn-info" data-dismiss="modal"><?php echo get_phrase('cancel'); ?></button>
            </div>
        </div>
    </div>
