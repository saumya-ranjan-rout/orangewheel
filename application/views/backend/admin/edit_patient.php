...
?>
  ...
  <option value="other" <?php if($row['sex']=="other"){ echo"selected"; } ?>>Other</option>
    ...
    <option value="appointment" <?php if($row['visit_type']=="appointment"){ echo"selected"; } ?>>Appointment</option>
        ...
        <div class="form-group">
    <label for="field-1" class="col-sm-3 control-label"><?php echo get_phrase('visit_date'); ?> <small style="color:red;">*</small> </label>
    
    <div class="col-sm-7">
        <?php
        $date = DateTime::createFromFormat('Y-m-d', $row['visit_date']);
...
if ($date === false) {
    ...
    $date_in_display_format = $date->format('m/d/Y');

}
?>
    <input type="text" name="visit_date" class="form-control datepicker" id="visit_date" value="<?php echo $date_in_display_format; ?>" required>
    </div>
    </div>
