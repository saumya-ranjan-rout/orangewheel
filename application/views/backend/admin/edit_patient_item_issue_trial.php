...
<ol class="breadcrumb bc-3" style="margin-bottom: 0px;">
    ...
    <li><?php echo get_phrase('patient_item_issue_edit')." Trial" ?></li>
</ol>
<br>
...
foreach($models as $model) { ?>
                ...
                <div class="row" style="margin-top: 10px;">
                    <div class="col-sm-2">
                        ...
                        <select name="model_id[]" class="form-control select2 srr" onchange="get_model_available_quantity(this.value, <?php echo $i; ?>)" required  style="color: #999; background-color: #868686; pointer-events: none;"  id="model_id_<?php echo $i; ?>">
...
<script>
   ...
   $.ajax({
                url: '<?php echo site_url('admin/get_demo_model_available_quantity/'); ?>' + selectedModelId,
...
models.forEach(function(model) {
    ...
    function get_model_price(item_price_details_id, append_id) {
      ...
      if (item_price_details_id != '') {
          $.ajax({
              url: '<?php echo site_url('admin/get_demo_model_price/'); ?>' + item_price_details_id,
              success: function(response) {
                  var data = JSON.parse(response);
