        ...
        <form role="form" class="form-horizontal form-groups" action="<?php echo site_url('admin/patient_item_issue/mainupdate'); ?>" method="post" enctype="multipart/form-data">
            <div class="row">
                <div class="col-sm-3">
                    ...
                    <select name="issue_type" class="form-control" id="issue_type" required onchange="get_invoice_no(this.value)" style="background-color: #f2f2f2; color: #808080; cursor: not-allowed; pointer-events: none; user-select: none;">
...
foreach($models as $model) { ?>
         ...
         <span id="after_discount_1"> <?php $after_amount =$result['unit_price']-$model['discount']; echo "<b>(".$result['unit_price'] . "-" . $model['discount'] . "=" .$after_amount.")</b>"; ?></span>
                    ...
                    <div class="col-sm-2">
                        ...
                        <input type="hidden" class="form-control" id="model_main_id_<?php echo $i; ?>" value="<?php echo $model["model_id"]; ?>" readonly />
                    </div>
                    <!-- <div class="col-sm-1">
...
//                                     </option>
                 ...
                 <div class="row" style="margin-top: 10px;">
                    ...
                    <label><?php echo get_phrase('total_price'); ?></label>
                        ...
                        <input type="hidden" class="form-control" id="model_main_id" value="" readonly />
                    </div>
                    <div class="col-sm-1">
            ...
            <div class="row" align="right" <?php if ($patient_item_issue_info['coupon_value'] == 0 || $patient_item_issue_info['coupon_value'] == NULL) { echo 'style="display:none; margin-top: 10px;"'; } else { echo 'style="margin-top: 10px;"'; } ?>>


                <label for="field-ta" class="col-sm-10 control-label"><a class="btn btn-danger" id="base-tab1"  disabled><i class="fa fa-tag"></i>
                                        Coupon </a></label>
                ...
                <div class="col-sm-2">
                    <div class="tab-pane" id="tab1" >
                                    <div class="input-group">
                                        ...
                                        <input type="text" class="form-control" id="coupon" name="coupon" value="<?php echo $patient_item_issue_info['coupon']; ?>" readonly><span class="input-group-addon round" style=" padding: 0px 12px;
  font-size: 12px; font-weight: normal; line-height: 1;  color: #555555; text-align: center; background-color: #fff;  border: none; border-radius: 3px;"> <a class="apply_coupon btn btn-small btn-primary sub-btn">Apply</a></span>
                        ...
                        <div class="col-sm-11">
                            <div class="date-and-time">
                                 <input type="number" name="coupon_perc" class="form-control per" style="float: left; width: 30%;"required="" value="<?php if($patient_item_issue_info['coupon_perc'] != NULL || $patient_item_issue_info['coupon_perc'] != 0) { echo $patient_item_issue_info['coupon_perc']; } else{ echo 0; } ?>" readonly>
                                <input type="text" name="coupon_value" class="form-control"  style="float: left;  width: 70%;" value="<?php echo $patient_item_issue_info['coupon_value']; ?>" readonly>
            ...
            <div class="row" align="right" style="margin-top: 10px;" >
                ...
                <div class="col-sm-2">
                    <input type="text" name="received_amount" class="form-control" value="<?php echo $patient_item_issue_info['received_amount']; ?>" id="received_amount" onkeyup="validateAndCalculate()" />
                    </div>
            </div>
...
function validateInput(input) {
    return /^[+-]?\d*\.?\d*$/.test(input); // Allow the dot to be optional and match zero or more digits after it
}
function validateAndCalculate() {
    var receivedAmountInput = document.getElementById("received_amount");
    var receivedAmount = receivedAmountInput.value;
    if (!validateInput(receivedAmount)) {
...
function get_invoice_no(type) {
    ...
    if (type == 'challan') {
        ...
        $('.sk').each(function(index) {
    var currentIndex = index + 1;
    var model_main_id = $('#model_main_id_' + currentIndex).val();

    var options = '<option value="">Select A Model</option>';
    <?php foreach ($item_info as $row): ?>
        <?php if ($row['demo'] == 0): ?>
            var model_id = '<?php echo $row['id']; ?>'; // Store model ID in a JavaScript variable
            var model_name = '<?php echo addslashes($row['model']); ?>'; // Escape special characters in model name
            options += '<option value="' + model_id + '" ' + (model_main_id == model_id ? 'selected' : '') + '>' + model_name + '</option>';
        <?php endif; ?>
    <?php endforeach; ?>
...
// $('#model_price_' + append_id).attr('value', data.unit_price);
            ...
            alert('Model Quantity is not available');
                   ...
                   $('#model_quantity_' + append_id).attr('max', 1);
  // $('#model_price_' + append_id).attr('value', "");
                   $('#model_old_price_' + append_id).attr('value', "");                   $('#model_serial_no_' + append_id).val("");
                                    calculate_total_price(append_id);
                                } 
                 // $('#model_price_'+ append_id).attr('value', data.unit_price);
  ...
  //    $('#model_price_' + append_id).attr('value', "");
    ...
    function add_model() {
        ...
        $('#model_main_id').attr('id', 'model_main_id_' + model_count);
...
var finalTotal = grandTotal - coupon_value;
var due_amount = finalTotal - received_amount;
    ...
    //     var grand_total = $('#grand_total').val();
    //     var received_amount = $('#received_amount').val();
    //     var due_amount = grand_total - received_amount;
