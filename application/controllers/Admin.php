...
class Admin extends CI_Controller
{
    ...
    // ADMIN DASHBOARD
      ...
      function doctor_commission_report()
    {
        if ($this->session->userdata('admin_login') != 1) {
        ...
        if($this->input->post()){
            ...
            $date_to = date('d-m-Y', strtotime($this->input->post('date_to')));
              $referred_by = $this->input->post('doctor');
            $page_data['referred_by'] = $referred_by;
            $doctor_commission_report  = $this->crud_model->doctor_commission_report_info($referred_by);                                                  
        }else{            
            $doctor_commission_report = $this->crud_model->doctor_commission_report_info_all();
        }
        
        $page_data['date_from'] = $date_from;
        $page_data['date_to'] =  $date_to;
        $page_data['doctor_commission_report'] = $doctor_commission_report;
          $page_data['doctor_name'] = $this->crud_model->doctor_name_info();
        $page_data['page_name']  = 'doctor_commission_report';
        $page_data['page_title'] = get_phrase('doctor_commission_report');
        $this->load->view('backend/index', $page_data);
    }
    
    
     function doctor_commission_history()
     {
          if ($this->session->userdata('admin_login') != 1) {
            $this->session->set_userdata('last_page', current_url());
            redirect(site_url(), 'refresh');
       ...
       $referred_by = $this->input->get('referred_by');
            $page_data['referred_by'] = $referred_by;
              $payment_date = $this->input->get('payment_date');
            $page_data['payment_date'] = $payment_date;
            
             $referred_id = $this->input->get('referred_id');
            $page_data['referred_id'] = $referred_id;
            
            

     $diag = $this->crud_model->doctor_commission_paid_diag($referred_by, $payment_date, $referred_id);  
     $page_data['diag'] = $diag;
     
      $item = $this->crud_model->doctor_commission_paid_item($referred_by, $payment_date, $referred_id);  
     $page_data['item'] = $item;
     
         $page_data['page_name']  = 'doctor_commission_history';
        $page_data['page_title'] = get_phrase('doctor_commission_history');
        $this->load->view('backend/index', $page_data);
...
if ($duplicate_check) {
 ...
 if($this->input->post()){
   ...
   function demoitem($task = "", $item_id = "")
    {
        if ($this->session->userdata('admin_login') != 1) {
            $this->session->set_userdata('last_page', current_url());
            redirect(site_url(), 'refresh');
        ...
        if ($task == "create") {
            $this->crud_model->save_demoitem_info();
            $this->session->set_flashdata('message', get_phrase('demoitem_info_saved_successfuly'));
            redirect(site_url('admin/demoitem'), 'refresh');
        }

        if ($task == "update") {
            $this->crud_model->update_demoitem_info($item_id);
            redirect(site_url('admin/demoitem'), 'refresh');
        }

        if ($task == "delete") {
            $this->crud_model->delete_demoitem_info($item_id);
            redirect(site_url('admin/demoitem'), 'refresh');
        }
        if ($this->input->post()) {
            $data['post_item_category'] = $this->input->post('item_category');
        ...
        $data['item_subcategory'] = $this->crud_model->select_item_subcategory_info();
        $data['item_category'] = $this->crud_model->select_item_category_info();
        $data['item_info'] = $this->crud_model->select_demoitem_info();
        $data['page_name']         = 'manage_demoitem';
        $data['page_title']        = 'Demo Item';
        $this->load->view('backend/index', $data);
    }
    function change_demoprice($item_id = "")
    {
        if ($this->session->userdata('admin_login') != 1) {
            $this->session->set_userdata('last_page', current_url());
            redirect(site_url(), 'refresh');
        ...
        $item_info = $this->crud_model->demoitem_details($item_id);
        $model_name = isset($item_info[0]['model']) ? $item_info[0]['model'] : '';
        $data['item_id'] =  $item_id;
        $data['page_name']    = 'change_demoprice';
        $data['page_title']   = get_phrase('demo_product_name') . ' : ' . $model_name;
        $this->load->view('backend/index', $data);
    }
    ...
    function create_patient_item_issue($task = "", $param2 = "")
    {
        ...
        function create_patient_item_issue_trial($task = "", $param2 = "")
    {
        if ($this->session->userdata('admin_login') != 1) {
            $this->session->set_userdata('last_page', current_url());
            redirect(site_url(), 'refresh');
        ...
        $data['item_info'] = $this->crud_model->select_demoitem_info();
        $data['page_name']  = 'add_patient_item_issue_trial';
        $data['page_title'] = get_phrase('add_patient_item_issue')." Trial";
        $this->load->view('backend/index', $data);
    }
   ...
   function patient_item_issue_report()
    ...
    function patient_item_challan_report()
    {
     ...
     function get_model_available_quantity_04_04_2024($item_id = '')
    {
     ...
     function get_model_available_quantity_17_04_2024($item_id = '')
    {
        $this->db->select('item.*,item_category.item_category');
        $this->db->from('item');
        $this->db->join('item_category', 'item_category.id = item.item_category_id', 'left');
        ...
        $this->db->select('item_price_details.*');
        $this->db->from('item_price_details');
        $this->db->where('item_price_details.item_id', $item_id);
        $query2 = $this->db->get();
        $options = $query2->result_array();
      ...
      function get_model_available_quantity($item_id = '')
    {
        ...
        $this->db->select('item_price_details.*');
        $this->db->from('item_price_details');
        $this->db->where(array('item_price_details.item_id'=>$item_id)); 
        $query2 = $this->db->get();
        $options = $query2->result_array();
     ...
     function check_coupon()
    {
        ...
        $this->db->select('*');
        $this->db->from('patient_item_issue');
        $this->db->where('patient_item_issue.coupon', $coupon);
        $query = $this->db->get();
        $num_rows = $query->num_rows(); // Get the number of rows returned by the query
    
        // Return the number of rows found by the query
        echo json_encode($num_rows);
    }
    
        function get_demo_model_available_quantity($item_id = '')
    {
        ...
        $this->db->where('demoitem.id', $item_id);
        $query = $this->db->get();
        $result = $query->row_array();

        $this->db->select('demoitem_price_details.*');
        $this->db->from('demoitem_price_details');
        $this->db->where('demoitem_price_details.item_id', $item_id);
        $query2 = $this->db->get();
        $options = $query2->result_array();
        ...
        function get_model_price($item_price_details_id = '')
    {

        $this->db->select('item_price_details.*');
        $this->db->from('item_price_details');
        $this->db->where('item_price_details.id', $item_price_details_id);
        $query = $this->db->get();
        $result = $query->row_array();
        echo json_encode($result);
    }
    
        function get_demo_model_price($item_price_details_id = '')
    {

        $this->db->select('demoitem_price_details.*');
        $this->db->from('demoitem_price_details');
        $this->db->where('demoitem_price_details.id', $item_price_details_id);
        $query = $this->db->get();
        $result = $query->row_array();
        echo json_encode($result);
    }
...
$data['date'] = $formatted_date;*/
         ...
         $data['date'] = DateTime::createFromFormat('m/d/Y', $this->input->post('creation_timestamp'))->format('Y-m-d');
        ...
        if ($this->session->userdata('admin_login') != 1) {
         ...
         if ($task == "update") {
              $doctor_name = $this->input->post('doctor_name');
               $doctor_name_new = $this->input->post('doctor_name_new');
        $this->crud_model->update_doctor_name($doctor_name,$doctor_name_new);
        $this->session->set_flashdata('message', get_phrase('data_updated_successfully'));
            redirect(site_url('admin/doctor_history'), 'refresh');
...
function patient_item_issue_17_04_2024($task = "", $param2 = "")
{
    if ($this->session->userdata('admin_login') != 1) {
        $this->session->set_userdata('last_page', current_url());
        redirect(site_url(), 'refresh');
...
function patient_item_issue($task = "", $param2 = "")
{
    ...
    if ($task == "create") {
        ...
        if ($this->input->post('issue_type') == 'challan') {
            ...
            redirect(site_url('admin/patient_item_issue/trial_list'), 'refresh');
        }else {
            redirect(site_url('admin/patient_item_issue/money_receipt_list'), 'refresh');
        }
    }
    ...
    if ($task == "mainupdate") {
        ...
        if ($task == "payment") {
        $this->crud_model->patient_item_issue_payment();
        $this->session->set_flashdata('message', get_phrase('payment_successful'));

            redirect(site_url('admin/patient_item_issue_pay/icd/'.$param2), 'refresh');
    ...
    if ($task == "invoice_list") {
        ...
        $data['list']  = 'money_receipt';
        $data['page_name']  = 'manage_money_receipt';
    } else if ($task == "trial_list") {
        $data['page_title'] = get_phrase('patient_item_issue')." Trial";
        $data['list']  = 'demo';
        $data['page_name']  = 'manage_patient_item_issue_trial_i';
...
function patient_item_issue_trial($task = "", $param2 = "")
{
    if ($this->session->userdata('admin_login') != 1) {
        $this->session->set_userdata('last_page', current_url());
        redirect(site_url(), 'refresh');
    ...
    if ($task == "create") {
        $this->crud_model->save_patient_item_issue_info_trial();
        $this->session->set_flashdata('message', get_phrase('data_added_successfully'));
      if ($this->input->post('issue_type') == 'invoice')  {
            redirect(site_url('admin/patient_item_issue_trial/invoice_list'), 'refresh');
        }
    }
    ...
    if ($task == "mainupdate") {
        $this->crud_model->update_main_patient_item_issue_info_trial();
        $this->session->set_flashdata('message', get_phrase('data_updated_successfully'));
...
function main_edit_patient_item_issue_trial($task ="",$id = "")
{
    if (!$this->session->userdata('admin_login')) {
        $this->session->set_userdata('last_page', current_url());
        redirect(site_url(), 'refresh');
    ...
    $data['item_info'] = $this->crud_model->select_demoitem_info();
    $data['patient_item_issue_info'] = $this->crud_model->select_patientitemissue_info($id);
  if ($task == "invoice_edit") {
  
        $data['page_title'] = get_phrase('patient_item_issue_edit')." Trial";
        $data['page_name']  = 'edit_patient_item_issue_trial';
    }
    $this->load->view('backend/index', $data);
}


function delete_patient_item_issue_17_04_2024($task ="",$id = "")
{
    if (!$this->session->userdata('admin_login')) {
        $this->session->set_userdata('last_page', current_url());
        redirect(site_url(), 'refresh');
...
function delete_patient_item_issue($task ="",$id = "")
{
    ...
    if ($task == 'challan') {
        ...
        redirect(site_url('admin/patient_item_issue/invoice_list'), 'refresh');
    }else if ($task == 'sale_return')  {
        redirect(site_url('admin/sales_return_list/sales_return'), 'refresh');
    }else {
        redirect(site_url('admin/patient_item_issue/money_receipt_list'), 'refresh');
    }
}

function delete_patient_item_issue_trial($task ="",$id = "")
{
    if (!$this->session->userdata('admin_login')) {
        $this->session->set_userdata('last_page', current_url());
        redirect(site_url(), 'refresh');
 ...
 function change_price($item_id = "")
    {
        ...
        $item_info = $this->crud_model->item_details($item_id);
        $model_name = isset($item_info[0]['model']) ? $item_info[0]['model'] : '';
        $data['item_id'] =  $item_id;
        $data['page_name']    = 'change_price';
        $data['page_title']   = get_phrase('product_name') . ' : ' . $model_name;
        $this->load->view('backend/index', $data);
    }
    function change_item_price($task = "", $item_id = "")
    {
        if ($this->session->userdata('admin_login') != 1) {
            $this->session->set_userdata('last_page', current_url());
            redirect(site_url(), 'refresh');
        ...
        if ($task == "create") {
            ...
            $this->crud_model->save_item_price();
            // $this->session->set_flashdata('message', get_phrase('item_price_details_saved_successfuly'));
            redirect(site_url('admin/change_price/' . $item_id), 'refresh');
        } 

        if ($task == "update") {
            $itemm_id = $this->input->post('item_id');
            $this->crud_model->update_item_price($item_id);
            redirect(site_url('admin/change_price/' . $itemm_id), 'refresh');
        }
        
        if ($task == "delete") {
            $this->crud_model->delete_item_price($item_id);
            $this->session->set_flashdata('message', get_phrase('price_details_deleted_successfuly'));
            redirect(site_url('admin/item'), 'refresh');
        }
    }
    
    public function doctor_referral1() 
    {
    $diag_grand_total = $this->input->post('diag_grand_total');
    $diag_com_per = $this->input->post('commissionPercentage1');
    $diag_com_amount = $this->input->post('calculatedCommissionAmount1');
    $item_grand_total = $this->input->post('item_grand_total');
    $item_com_per = $this->input->post('commissionPercentage2');
    $item_com_amount = $this->input->post('calculatedCommissionAmount2');
    $payment_date = date('d-m-Y');

    // Prepare data arrays
    $referral_data = array(
        'doctor_name' => $this->input->post('doctor_name'),
        'diag_grand_total' => $diag_grand_total,
        'diag_com_per' => $diag_com_per,
        'diag_com_amount' => $diag_com_amount,
        'item_grand_total' => $item_grand_total,
        'item_com_per' => $item_com_per,
        'item_com_amount' => $item_com_amount,
        'payment_date' => $payment_date
    );

    // Process referral details
    $pat_diag_id = $this->input->post('pat_diag_id');
    $pat_item_issue_id = $this->input->post('pat_item_issue_id');
    $patient_id = $this->input->post('patient_id');
    $date_time =  date('d-m-Y');

    $referral_details = array();
    foreach ($pat_diag_id as $key => $value) {
        $referral_details[] = array(
            'pat_diag_id' => $value,
            'pat_item_issue_id' => $pat_item_issue_id[$key],
            'patient_id' => $patient_id[$key],
            'date_time' => $date_time
        );
    }

    // Combine referral data and details into a single array
    $result = array(
 ...
 public function doctor_referral_05_04_2024() {
    $diag_grand_total = $this->input->post('diag_grand_total');
    $diag_com_per = $this->input->post('commissionPercentage1');
    $diag_com_amount = $this->input->post('calculatedCommissionAmount1');
    $item_grand_total = $this->input->post('item_grand_total');
    ...
    // Insert data into doctor_referral table
    $referral_id = $this->crud_model->insert_referral($referral_data);
   //  $patient_id = $this->input->post('patient_id');

    
    
 // Process referral details for diagnosis
    $pat_diag_id = $this->input->post('pat_diag_id');
    $date_time = date('d-m-Y');
   
   /*   foreach ($pat_diag_id as $key => $value) {
    $referral_details_data = array(
         'doctor_referral_id' => $referral_id,
        'pat_diag_id' => $value,
        'date_time' => $date_time
    );
    */

/*foreach ($pat_diag_id as $key => $pat_diag_ids_row) {
    
    if ($this->input->post('checkbox_name_' . $key)) {
        foreach ($pat_diag_ids_row as $pat_diag_id) {
            $referral_details_data = array(
                'doctor_referral_id' => $referral_id,
                'pat_diag_id' => $pat_diag_id,
                'date_time' => $date_time
            );
            
      
    
            $this->crud_model->insert_referral_details_diagnosis($referral_details_data);
        }
   }
}*/
...
foreach ($pat_diag_id as $key => $pat_diag_ids_row) {
    echo 'Checkbox name: checkbox_name_' . $key . ', Value: ' . $this->input->post('checkbox_name_' . $key) . '<br>';
    if ($this->input->post('checkbox_name_' . $key)) {
      if (empty($pat_diag_ids_row)) {
    echo 'The $pat_diag_ids_row array is empty.';
} else {
    foreach ($pat_diag_ids_row as $pat_diag_id) {
        $pat_diag_id_json = json_encode($pat_diag_id);
        echo 'Inside inner loop: ' . $pat_diag_id_json . '<br>'; // Debugging output
    }
}
  ...
  /*  foreach ($pat_diag_id as $key => $value) {
        ...
        // Insert data into doctor_referral_details for diagnosis
        $this->crud_model->insert_referral_details_diagnosis($referral_details_data);
    }*/

    // Process referral details for item issues
  /*  $pat_item_issue_id = $this->input->post('pat_item_issue_id');

    foreach ($pat_item_issue_id as $key => $value) {
        $referral_details_data = array(
            'doctor_referral_id' => $referral_id,
            'pat_item_issue_id' => $value,
           // 'patient_id' => $patient_id[$key],
            'date_time' => $date_time
        );
       
        $this->crud_model->insert_referral_details_item_issue($referral_details_data);
    }*/

    // Redirect or set flash message if needed
}


public function doctor_referral_17_04_2024() {
    $diag_grand_total = $this->input->post('diag_grand_total');
    $diag_com_per = $this->input->post('commissionPercentage1');
    $diag_com_amount = $this->input->post('calculatedCommissionAmount1');
    $item_grand_total = $this->input->post('item_grand_total');
...
foreach ($checked as $key => $chk) {
         if ($chk == 1)  {
   $pt_id = $patient_id[$key];
    $pat_diag_ids_row = $this->input->post('pat_diag_id'.$pt_id);
    $pat_item_issue_id = $this->input->post('pat_item_issue_id'.$pt_id);



         foreach ($pat_diag_ids_row as $key1 => $pat_diag_id) {
            $referral_details_data = array(
                'doctor_referral_id' => $referral_id,
                'pat_diag_id' => $pat_diag_id,
                'patient_id' => $pt_id,
                'date_time' => $date_time
                 ...
                 $doctor_referral_details_diagnosis_id = $this->crud_model->insert_referral_details_diagnosis($referral_details_data);

                 if($doctor_referral_details_diagnosis_id) {
                    $details_data_patient_diagnosis = array(
                        'referral_status' => 'Done',
                        'referral_date' => $date_time,
                        'referral_doctor' => $doctor_name
                    );
                    $this->db->where('id', $pat_diag_id);
                    $this->db->update('patient_diagnosis', $details_data_patient_diagnosis);
                 }


         }

         foreach ($pat_item_issue_id as $key2 => $pat_item_id) {
            $referral_details_data = array(
                'doctor_referral_id' => $referral_id,
                'pat_item_issue_id' =>  $pat_item_id,
                'patient_id' => $pt_id,
                'date_time' => $date_time
            );
            $doctor_referral_details_item_issue_id = $this->crud_model->insert_referral_details_item_issue($referral_details_data);

            if($doctor_referral_details_item_issue_id) {
                $details_data_patient_diagnosis = array(
                    'referral_status' => 'Done',
                    'referral_date' => $date_time,
                    'referral_doctor' => $doctor_name
                );
                $this->db->where('id', $pat_item_id);
                $this->db->update('patient_item_issue', $details_data_patient_diagnosis);
             }
...
public function doctor_referral() {
    $diag_grand_total = $this->input->post('diag_grand_total');
    $diag_com_per = $this->input->post('commissionPercentage1');
    $diag_com_amount = $this->input->post('calculatedCommissionAmount1');
    $item_grand_total = $this->input->post('item_grand_total');
    ...
    foreach ($pat_diag_ids_row as $key1 => $pat_diag_id) {
        if ($chk == 1) {
         ...
         foreach ($pat_item_issue_id as $key2 => $pat_item_id) {
            ...
            if($doctor_referral_details_item_issue_id) {
                $details_data_patient_item_issue = array(
                    'referral_status' => 'Done',
                    'referral_date' => $date_time,
                    'referral_doctor' => $doctor_name,
                    'status' => 'Paid'
                ...
                $this->db->where('id', $pat_item_id);
                $this->db->update('patient_item_issue', $details_data_patient_item_issue);
             }
            } else {
                $details_data_patient_item_issue = array(
                    'status' => 'Unpaid'
...
function sales_return_list($task = "",$id = "")
    {
        if ($this->session->userdata('admin_login') != 1) {
            $this->session->set_userdata('last_page', current_url());
            redirect(site_url(), 'refresh');
 ...
 if ($task == "create") {
       $data['page_name']    = 'manage_patient_details';
        $data['page_title']   = get_phrase('view_patient_for_sales_return');
 } else if ($task == "sales_return") {
            $data['page_title'] = get_phrase('sales_return_list');
            $data['list']  = 'sales_return';
            $data['page_name']  = 'manage_sales_return';
        } else  if ($task == "sales_return_create") {
        $data['page_title'] = get_phrase('sales_return_create');
        $data['page_name']  = 'create_sales_return';
    }
        
         if ($task == "return_update") {
        $this->crud_model->update_sales_return_patient_item_issue_info();
        $this->session->set_flashdata('message', get_phrase('data_updated_successfully'));
        if ($this->input->post('issue_type') == 'invoice')  {
            redirect(site_url('admin/sales_return_list/sales_return'), 'refresh');
        }
...
function change_demoitem_price($task = "", $item_id = "")
    {
        if ($this->session->userdata('admin_login') != 1) {
            $this->session->set_userdata('last_page', current_url());
            redirect(site_url(), 'refresh');
        ...
        if ($task == "create") {
            $item_id = $this->input->post('item_id');
            $this->crud_model->save_demoitem_price();
            // $this->session->set_flashdata('message', get_phrase('item_price_details_saved_successfuly'));
            redirect(site_url('admin/change_demoprice/' . $item_id), 'refresh');
        }

        if ($task == "update") {
            $itemm_id = $this->input->post('item_id');
            $this->crud_model->update_demoitem_price($item_id);
            redirect(site_url('admin/change_demoprice/' . $itemm_id), 'refresh');
        }

        if ($task == "delete") {
            $this->crud_model->delete_demoitem_price($item_id);
            $this->session->set_flashdata('message', get_phrase('demoprice_details_deleted_successfuly'));
            redirect(site_url('admin/demoitem'), 'refresh');
        }
    }
    ...
    function patient_item_issue_pay($task ="",$id = "")
{
    if (!$this->session->userdata('admin_login')) {
        $this->session->set_userdata('last_page', current_url());
        redirect(site_url(), 'refresh');
...
//         $data['page_name']  = 'edit_money_receipt';
//     }else{
        $data['page_title'] = get_phrase('patient_item_issue_payment');
        $data['page_name']  = 'patient_item_issue_pay';
  //  }
    $this->load->view('backend/index', $data);
}
} 