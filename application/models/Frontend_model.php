...
class Frontend_model extends CI_Model {
    ...
    // }
        ...
        if (isset($_FILES["icon"]) && !empty($_FILES['icon']['name'])) {
            ...
            $attachment_name = $service_id . '.' . $fileInfo['extension'];
    
            move_uploaded_file($_FILES["icon"]["tmp_name"], "./uploads/frontend/service_images/" . $attachment_name);
    
            $dataa = array('image' => $attachment_name);
            $this->db->where('service_id', $service_id);
    ...
    function send_contact_message() {
        ...
        if( $this->db->insert('contact_email', $data)){
            return true;
        } else {
            return false;
        }
